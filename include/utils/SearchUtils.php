...
function getAdvancedSearchCriteriaList($advft_criteria, $advft_criteria_groups, $module='') {
	...
	$moduleFields = array();
    $moduleFields[$module] = $moduleMeta->getModuleFields();

    /* Add swarnat ref search */
    $alreadyGetModuleFields = array();
    /* Add swarnat ref search */
	...
	foreach($advft_criteria as $column_index => $column_condition) {
        ...
        if(strpos($adv_filter_column, "->") !== false) {
            $isRefField = true;
            $parts = explode("->", $adv_filter_column);
            $refModule = $parts[1];
            $adv_filter_column = $parts[0];

            $refModuleParts = explode(":", $refModule);
            ...
            if(!in_array($refModuleParts[1], $alreadyGetModuleFields)) {
                ...
                $alreadyGetModuleFields[] = $refModuleParts[1];

                $moduleHandler = vtws_getModuleHandlerFromName($refModuleParts[1],$current_user);
               	$moduleMeta = $moduleHandler->getMeta();
                $moduleFields[$refModuleParts[1]] = $moduleMeta->getModuleFields();
            }
            $targetModule = $refModuleParts[1];
        } else {
            $targetModule = $module;
            $refModule = false;
        }
        ...
        $fieldObj = $moduleFields[$targetModule][$fieldName];
        if(empty($fieldObj)) {
            var_dump($column_info, $targetModule, $fieldName);
        }
    ...
    $advfilterlist["refmodule"] = $refFields;
