...
class QueryGenerator {
	...
	public function getFromClause() {
        ...
        foreach($this->ref_fields as $refField) {
            $parts = explode(":", $refField);

            $meta = $this->getMeta($parts[1]);
            $tables = $meta->getEntityTableIndexList();
            $joinField = $moduleFields[$parts[0]];

            foreach($tables as $table => $indexColumn) {
                $tableName = $table."_".$parts[0];
                if($table == "vtiger_crmentity") {
                    $tableJoinMapping[$tableName] = "INNER JOIN";
                } else {
                    $tableJoinMapping[$tableName] = "LEFT JOIN";
                }

                $tableJoinCondition[$parts[0]][$tableName] = $tableName.".".$indexColumn." = ".$joinField->getTableName().".".$joinField->getColumnName();
        ...
        foreach ($tableJoinCondition as $fieldName=>$conditionInfo) {
			foreach ($conditionInfo as $tableName=>$condition) {
                ...
                if(!empty($tableAliases[$tableName])) {
                    $tableNameAlias = $tableName;
                    $tableName = $tableAliases[$tableName];

                    $sql .= " $tableJoinMapping[$tableNameAlias] $tableName $tableNameAlias ON $condition";
                } else {
	...
	public function getWhereClause() {
        ...
        $alreadyGetFields = array();
        $moduleFieldList = array();
		$moduleFieldList[$this->module] = $this->meta->getModuleFields();
		...
		foreach ($this->conditionals as $index=>$conditionInfo) {
            ...
            if($conditionInfo["ref_field"] !== false) {
                $parts = explode(":", $conditionInfo["ref_field"]);
                if(!in_array($parts[1], $alreadyGetFields)) {
                    $alreadyGetFields[] = $parts[1];

                    $moduleHandler = $this->getMeta($parts[1]);

                    $moduleFields[$parts[1]]["_base"] = $moduleHandler->getModuleFields();
                }

                $moduleFieldList[$parts[1]][$parts[0]] = $moduleFields[$parts[1]]["_base"];

                $field = $moduleFieldList[$parts[1]][$parts[0]][$fieldName];

                $tableName = $field->getTableName()."_".$parts[0];
            } else {
			    $field = $moduleFieldList[$this->module][$fieldName];
	...
	public function addCondition($fieldname,$value,$operator,$glue= null,$newGroup = false,
			$newGroupType = null, $refField = false) {
	...
	private function getConditionalArray($fieldname,$value,$operator, $refField = false) {
		if(is_string($value)) {
			$value = trim($value);
		} elseif(is_array($value)) {
        ...
        if($refField !== false)
            $this->ref_fields[] = $refField;

		return array('name'=>$fieldname,'value'=>$value,'operator'=>$operator, 'ref_field' => $refField);
	}
	...
	public function addUserSearchConditions($input) {
		...
		if($input['searchtype']=='advance') {
			...
			foreach ($advfilterlist as $groupindex=>$groupcolumns) {
                if($groupindex == "refmodule") continue;
				...
				if(count($filtercolumns) > 0) {
					...
					foreach ($filtercolumns as $index=>$filter) {
						...
						$this->addCondition($name, $filter['value'], $filter['comparator'], null, false, null, $filter["ref_field"]);
                        $columncondition = $filter['column_condition'];
						if(!empty($columncondition)) {
							$this->addConditionGlue($columncondition);
						}
